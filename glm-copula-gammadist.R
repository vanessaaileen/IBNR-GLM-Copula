...
fit3incr <- glmReserve(data3incr, var.power = 2, cum = FALSE, mse.method = "bootstrap")
summary(fit3incr)
...
tri1up <- tri1[tri1$origin + tri1$dev <= 11, ]
beta1_a <- predict(gamma_glm1, data=tri1up)
beta1_a <- exp(beta1_a)
beta1_a <- beta1_a / alpha1$alpha
dataa <- pgamma(tri1up$value, shape = alpha1$alpha, scale = beta1_a)
...
tri2up <- tri2[tri2$origin + tri2$dev <= 11, ]
beta_b <- predict(gamma_glm2, data=tri2up)
beta_b <- exp(beta_b)
beta_b <- beta_b / alpha2$alpha
datab <- pgamma(tri2up$value, shape = alpha2$alpha, scale = beta_b)
...
tri3up <- tri3[tri3$origin + tri3$dev <= 11, ]
beta_c <- predict(gamma_glm3, data=tri3up)
beta_c <- exp(beta_c)
beta_c <- beta_c / alpha3$alpha
datac <- pgamma(tri3up$value, shape = alpha3$alpha, scale = beta_c)
...
set.seed(100)
for (i in 1:1000) {
  random <- rCopula(45, fit1copula@copula)
...
#plot 3d #######################################################################
u1 <- rCopula(55, fit1copula@copula)
u <- dataabc
